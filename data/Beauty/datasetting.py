...
df = pd.read_excel("Amazon_Beauty_Recommendation.xlsx", engine='openpyxl')
...
def get_last_three_items(group):
    return group.sort_values('timestamp')['item_id'].iloc[-3:].values
...
class_weekend = class_weekend[['user_id', 'item_id', 'rating', 'timestamp']]
class_workday = class_workday[['user_id', 'item_id', 'rating', 'timestamp']]
...
def split_dataset(df):
    ...
    for user_id, group in df.groupby('user_id_new'):
        items = group.sort_values('timestamp')['item_id_new'].values
        if len(items) >= 2:
...
with open("../../../IORec-g/SASRec.pytorch-main/Beauty.txt", "w") as f:
    for user_id, items in user_to_items.items():
        line = str(user_id) + " " + " ".join(map(str, items))
        f.write(line + "\n")

print("序列数据 Beauty.txt 已生成。")



weekend_df = pd.read_csv('../../data/Beauty/weekend/weekend.txt', sep='\t')
workday_df = pd.read_csv('../../data/Beauty/workday/workday.txt', sep='\t')


combined_df = pd.concat([weekend_df, workday_df], ignore_index=True)

iid_users = np.load('../../data/Beauty/iid_users.npy')
ood_users = np.load('../../data/Beauty/ood_users.npy')
