...
class DistSAModelTrainer(Trainer):
    ...
    def bpr_optimization(self, seq_mean_out, seq_cov_out, pos_ids, neg_ids):
        ...
        #######################
        # 计算序列中连续位置之间的正样本距离
        seq_pos = wasserstein_distance(seq_mean_emb[:-1], seq_cov_emb[:-1],
                                       seq_mean_emb[1:], seq_cov_emb[1:])

        # 计算序列中非连续位置之间的负样本距离
        seq_neg = wasserstein_distance(seq_mean_emb[:-1], seq_cov_emb[:-1],
                                       seq_mean_emb[1:].roll(1), seq_cov_emb[1:].roll(1))
        #
        # contrast_loss = -torch.log(torch.sigmoid(seq_neg - seq_pos)).mean()
        # #######################
        #
        #
        # istarget = (pos_ids > 0).view(pos_ids.size(0) * self.model.args.max_seq_length).float() # [batch*seq_len]
        ...
        contrast_loss = -F.logsigmoid(seq_neg - seq_pos).mean()

        istarget = (pos_ids > 0).view(-1).float()

        # 限制 logits 差值范围，防止极端值
        logits_diff = neg_logits - pos_logits
        logits_diff = torch.clamp(logits_diff, min=-20, max=20)  # 可根据经验调整

        # loss 计算用更稳定的 log-sigmoid
        loss = torch.sum(-F.logsigmoid(logits_diff) * istarget) / (torch.sum(istarget) + 1e-8)

        pvn_term = pos_logits - pos_vs_neg
        pvn_term = torch.clamp(pvn_term, min=0)
        pvn_loss = self.args.pvn_weight * torch.sum(pvn_term * istarget) / (torch.sum(istarget) + 1e-8)
