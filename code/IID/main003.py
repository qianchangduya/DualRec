...
class LossBasedEarlyStopping:
    ...
    def __init__(self, save_path, patience=5, verbose=False, delta=0.0001):
        ...
        self.best_test_preds = None  # 新增：保存测试集最佳预测
    ...
    def __call__(self, val_loss, model, preds=None, test_preds=None):
        ...
        if self.best_score is None:
            ...
            self.save_checkpoint(val_loss, model, preds, test_preds)
    ...
    def save_checkpoint(self, val_loss, model, preds=None, test_preds=None):
        """保存模型和预测结果"""
        if self.verbose:
        ...
        if test_preds is not None:
            self.best_test_preds = test_preds  # 新增：保存测试集预测结果
...
def main():
    ...
    if args.prediction_path is None:
        ...
        args.prediction_path = os.path.join(args.output_dir, f"Taobao_predictions.npy")
    ...
    if args.do_eval:
        ...
        early_stopping = LossBasedEarlyStopping(
            ...
            patience=5,  # 容忍损失不下降的轮数
            verbose=True,
            delta=0.0001  # 损失变化阈值
        )

        test_answer_list = None
        ...
        best_test_pred_list = early_stopping.best_test_preds  # 新增：获取测试集最佳预测
        ...
        if best_pred_list is not None and answer_list is not None:
            ...
            np.save(test_pred_path, best_test_pred_list)
            np.save(test_answer_path, test_answer_list)  # 使用最后获取的测试答案
