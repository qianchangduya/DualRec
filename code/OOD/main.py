...
parser.add_argument('--total_anneal_steps', type=int, default=200000,
                    help='the total number of gradient updates for annealing')  # 退火参数的总梯度更新步数
...
parser.add_argument('--CI', type=int, default=1,
                    help='whether use counterfactual inference in ood settings')  # 在OOD环境下启用反事实推理
...
# 加载 IID 数据
iid_users = np.load('../../data/Meituan/' + 'iid_users.npy', allow_pickle=True)
# 加载 OOD 数据
ood_users = np.load('../../data/Meituan/' + 'ood_users.npy', allow_pickle=True)
...
if args.ood_finetune:
    train_dataset = 'weekday'  # 训练数据集为OOD
    ood_test_dataset = 'workday'  # OOD测试数据集为IID
else:
    train_dataset = 'workday'  # 训练数据集为IID
    ood_test_dataset = 'weekday'  # OOD测试数据集为OOD
...
##########################
train_data = train_data[ood_users, :]
valid_x_data = valid_x_data[ood_users, :]
valid_y_data = valid_y_data[ood_users, :]
test_x_data = test_x_data[ood_users, :]
test_y_data = test_y_data[ood_users, :]
n_users = len(ood_users)
user_feature = user_feature[ood_users, :]

ood_train_data = ood_train_data[ood_users, :]
ood_valid_x_data = ood_valid_x_data[ood_users, :]
ood_valid_y_data = ood_valid_y_data[ood_users, :]
ood_test_x_data =  ood_test_x_data[ood_users, :]
ood_test_y_data = ood_test_y_data[ood_users, :]
ood_n_users = len(ood_users)
ood_user_feature = ood_user_feature[ood_users, :]
##########################
...
if args.ood_finetune:
    ...
    if args.model_name == 'COR':
        model = models.COR(mlp_q_dims, mlp_p1_dims, mlp_p2_dims, mlp_p3_dims, \
                           adj, E1_size, args.dropout, args.bn, args.sample_freq, args.regs,
...
if args.ood_finetune:  # 如果进行OOD微调
    ...
    if args.X == 0:  # 如果X为0
        ...
        _, test_results = evaluate(ood_test_x_data, ood_test_y_data, ood_test_x_data + ood_valid_y_data,
                                   ood_user_feature, eval(args.topN))
        evaluate_util.print_results(None, None, test_results)  # 打印测试结果

        print(f"Performance on {train_dataset}")  # 打印训练数据集的性能
        _, test_results = evaluate(ood_test_x_data, test_y_data, ood_test_x_data + ood_valid_y_data, user_feature,
                                   eval(args.topN), 1)
        evaluate_util.print_results(None, None, test_results)  # 打印测试结果
        print('-' * 18)
...
try:
    for epoch in range(1, args.epochs + 1):  # 进行多个训练轮次
        ...
        if epoch % evaluate_interval == 0:  # 每隔一定轮次进行评估
            ...
            print('---' * 18)
            print("Runing Epoch {:03d} ".format(epoch) + 'valid loss {:.4f}'.format(
                valid_loss) + " costs " + time.strftime(
            ...
            if valid_results[1][K] > best_recall:  # 根据召回率选择最佳模型
                ...
                if not args.ckpt is None:  # 如果指定了检查点，保存微调模型
                    torch.save(model,
                               '{}{}_{}_{}_{}_{}_{}_{}_{}_{}_{}lr_{}wd_{}bs_{}anneal_{}cap_{}CI_{}drop_{}_{}_{}_{}bn_{}freq_{}reg_{}_{}_ood_003.pth'.format(
                                   args.save_path, args.model_name, args.dataset, train_dataset, ckpt_structure[-20],
                                   ckpt_structure[-19], ckpt_structure[-18], \
                                   ...
                                   args.dropout, ckpt_structure[-7], ckpt_structure[-6], ckpt_structure[-5], args.bn,
                                   args.sample_freq, args.regs, ckpt_structure[-2], args.log_name))
                else:  # 否则保存完整模型
                    torch.save(model,
                               '{}{}_{}_{}_{}q_{}p11_{}p12_{}p2_{}p3_{}lr_{}wd_{}bs_{}anneal_{}cap_{}CI_{}drop_{}Z1_{}E2_{}Z2_{}bn_{}freq_{}reg_{}_{}_ood_003.pth'.format(
                                   args.save_path, args.model_name, args.dataset, train_dataset, args.mlp_dims,
                                   args.mlp_p1_1_dims, \
                                   args.mlp_p1_2_dims, args.mlp_p2_dims, args.mlp_p3_dims, args.lr, args.wd, \
                                   args.batch_size, args.total_anneal_steps, args.anneal_cap, args.CI, \
                                   args.dropout, args.Z1_hidden_size, args.E2_hidden_size, args.Z2_hidden_size, args.bn,
                                   args.sample_freq, args.regs, args.act_function, args.log_name))
