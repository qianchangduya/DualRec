...
ood_users = np.load('../../data/Taobao/' + 'ood_users.npy', allow_pickle=True)
# ood_users = np.load('../../data/ml1m/' + 'ood_users.npy', allow_pickle=True)
# ood_users = np.load('../../data/Beauty/' + 'ood_users.npy', allow_pickle=True)
...
def evaluate(data_tr, data_te, his_mask, user_feat, topN, CI=0):
    ...
    predict_items_ood = predict_items_np[ood_users]
    target_items_ood = target_items_np[ood_users]
    ...
    return total_loss, predict_items, target_items
...
try:
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % evaluate_interval == 0:
            ...
            valid_loss, valid_predict, valid_target = evaluate(valid_x_data, valid_y_data, mask_val, user_feature, eval(args.topN))
            test_loss, test_predict, test_target = evaluate(test_x_data, test_y_data, mask_test, user_feature, eval(args.topN))
            ...
            if valid_loss < best_valid_loss:
                ...
                print(f"[Epoch {epoch}] 验证损失下降，保存最佳模型及预测结果到 {save_dir}（验证损失：{valid_loss:.4f}）")
            else:
                patience_counter += 1
                print(f"[Epoch {epoch}] 验证损失未下降，耐心剩余：{patience - patience_counter}")
